...
/**
 * This class launches the analog widget configuration for Android12+ to choose whether to display the analog widget
 * with or without the second hand.
 * <p>
 * Earlier versions cannot display the second hand so this activity is not shown to the user.
 */
public class AnalogAppWidgetConfiguration extends AppCompatActivity {
    ...
    private void onWidgetContainerClicked(boolean isSecondHandDisplayed) {
        mPrefs.edit().putBoolean(KEY_ANALOG_WIDGET_WITH_SECOND_HAND, isSecondHandDisplayed).apply();

        AppWidgetManager wm = AppWidgetManager.getInstance(this);

        AnalogAppWidgetProvider.updateAppWidget(this, wm, mAppWidgetId);
