 ...
 */
public class VerticalAppWidgetProvider extends BaseDigitalAppWidgetProvider {

    @Override
    protected int getLayoutWithShadowId() {
        return R.layout.appwidget_vertical_with_shadow;
    }

    @Override
    protected int getLayoutWithoutShadowId() {
        return R.layout.appwidget_vertical;
    }

    @Override
    protected int getSizerLayoutWithShadowId() {
        return R.layout.appwidget_vertical_sizer_with_shadow;
    }

    @Override
    protected int getSizerLayoutWithoutShadowId() {
        return R.layout.appwidget_vertical_sizer;
    }

    @Override
    protected int getWidgetViewId() {
        return R.id.vertical_widget;
    }
    ...
    protected void configureClock(RemoteViews rv, Context context, SharedPreferences prefs) {
        if (WidgetDAO.isVerticalWidgetDefaultHoursColor(prefs)) {
            ...
            rv.setTextColor(getClockHoursCustomViewId(), WidgetDAO.getVerticalWidgetCustomHoursColor(prefs));
        }

        if (WidgetDAO.isVerticalWidgetDefaultMinutesColor(prefs)) {
            ...
            rv.setTextColor(getClockMinutesCustomViewId(), WidgetDAO.getVerticalWidgetCustomMinutesColor(prefs));
        }
    }
    ...
    protected void configureBackground(RemoteViews rv, Context context, SharedPreferences prefs, int widthPx, int heightPx) {
        ...
        boolean isDefaultBackgroundColor = WidgetDAO.isVerticalWidgetDefaultBackgroundColor(prefs);
        ...
        if (SdkUtils.isAtLeastAndroid12()) {
            ...
            if (isDefaultBackgroundColor) {
                final Icon backgroundIcon = Icon.createWithResource(context, R.drawable.appwidget_vertical_background);
    ...
    protected void configureSizerNextAlarm(View sizer, Context context, SharedPreferences prefs, String nextAlarmTime) {
        ...
        if (WidgetDAO.isVerticalWidgetDefaultNextAlarmColor(prefs)) {
            ...
            nextAlarmIconForCustomColor.setTextColor(WidgetDAO.getVerticalWidgetCustomNextAlarmColor(prefs));
        }
    }
